...
export function processFormUpdate(engine: Engine) {
  ...
  for (const entity of setTriggers) {
    const { tile, formSet, formSetTriggers } = entity
    formSetTriggers.forEach((tag, tagIndex) => {
      if (tag in entity) {
        const i = tagIndex * 3

        const newForm = [
          formSet[i] === '' ? tile.char : formSet[i],
          formSet[i + 1] === '' ? tile.color : formSet[i + 1],
          formSet[i + 2] === '' ? tile.bgColor : formSet[i + 2],
  ...
  for (const entity of autoCyclers) {
    const { tile, formSet, formSetAutoCycle: cycle } = entity
  ...
  for (const key of tAnimators) {
    ...
    const { tile, formSet, formSetAutoCycle: cycle } = terrain
    if (Date.now() - cycle.lastUpdate > cycle.frequency) {
      const nextI = cycle.current + 3
      const i = nextI >= formSet.length ? 0 : nextI
